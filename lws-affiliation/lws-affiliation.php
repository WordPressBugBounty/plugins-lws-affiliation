...
add_action('admin_notices', function () {
    if (substr(get_current_screen()->id, 0, 38) == "toplevel_page_lws-affiliation-settings") {
...
?>
    ...
    <div class="notice notice-info is-dismissible lws_aff_review_block_general" id="lws_aff_review_notice">
        ...
        <div style="padding:16px">
            ...
            <p class="lws_aff_review_block_desc"><?php esc_html_e('Evaluate our plugin to help others discover the affiliate program and earn money!', 'lws-affiliation'); ?></p>
...
function lws_aff_setup()
{
    ...
    if (isset($_POST['validate-config-aff-lws'])) {
        if (!empty(sanitize_text_field(wp_unslash($_POST['username-aff-lws']))) && !empty(sanitize_text_field(wp_unslash($_POST['password-aff-lws'])))) {
            if (is_numeric(sanitize_text_field(wp_unslash($_POST['username-aff-lws'])))) {
                $urlAuth = 'https://affiliation.lws-hosting.com/api/auth/%d/%s';
                $retourApi = wp_remote_get(sprintf($urlAuth, sanitize_text_field(wp_unslash($_POST['username-aff-lws'])), sanitize_text_field(wp_unslash($_POST['password-aff-lws']))));

                if ( is_array( $retourApi ) && ! is_wp_error( $retourApi ) ) {
                    $retourApi    = json_decode($retourApi['body']);
                } else {
                    $retourApi = false;
                }
                ...
                if (property_exists($retourApi, 'error')) {
                    ...
                    update_option('lws_aff_apikey', wp_json_encode($retourApi->apikey, JSON_PRETTY_PRINT));
                    $formSuccess = true;
                } else {
        ...
        } elseif (empty(sanitize_text_field(wp_unslash($_POST['username-aff-lws'])))) {
            $formError = esc_html__('Please enter your affiliate ID.', 'lws-affiliation');
        } elseif (empty(sanitize_text_field(wp_unslash($_POST['password-aff-lws'])))) {
...
function lws_aff_check_username()
{
    if (isset($_SERVER['QUERY_STRING'])) {
        ...
        if (!strstr($query_string, 'page=lws-affiliation-settings')) {
...
function lws_aff_apiStats()
{
    ...
    $retourApi = wp_remote_get($url, true);

    if ( is_array( $retourApi ) && ! is_wp_error( $retourApi ) ) {
        $retourApi    = json_decode($retourApi['body'], true);
    } else {
        $retourApi = false;
    }
...
function lws_aff_apiLastSales()
{
    ...
    $retourApi = wp_remote_get(sprintf($urlAuth, json_decode(get_option('lws_aff_apikey'))));
    if ( is_array( $retourApi ) && ! is_wp_error( $retourApi ) ) {
        $retourApi = json_decode($retourApi['body'], true);
    } else {
        $retourApi = false;
    ...
    return $retourApi['last_vente'] ?? false;
}
...
{
    check_ajax_referer('activate_plugins_lws_plugins', '_ajax_nonce');
                    ...
                    <?php foreach ($banners['widget'] as $k => $type) : ?>
                        <?php if ($k == 'domain_search') : ?>
                            ...
                            <?php foreach ($type as $widget) : ?>
                                <?php $get_url = sanitize_url(wp_unslash($_GET['url'] ?? '')); ?>
                                <div class="banner_widget">
                                    <div id="div-<?php echo esc_html($widget['id']); ?>" style="text-align:center;">
                                        ...
                                        <a class="preview_button" name="open_preview_button" value="<?php echo esc_html($widget['id']); ?>" target="_blank" href="<?php echo esc_url($get_url); ?>?action=load_preview_widget&_ajax_nonce=<?php echo esc_attr(wp_create_nonce('load_preview_widget_nonce')); ?>&id=<?php echo esc_html($widget['id']); ?>"><?php esc_html_e('Preview', 'lws-affiliation'); ?></a>
            ...
            <div>
                <?php foreach ($banners['widget'] as $k => $type) :
                    foreach ($type as $widget) :
                        $get_id = sanitize_text_field(wp_unslash($_GET['id'] ?? ''));
                        if ($widget['id'] == $get_id) :
